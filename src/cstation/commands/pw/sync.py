...
class PWSync:
    ...
    def sync_files(
        ...
        try:
            ...
            success = self._sync_pw_to_server(Path(), host, version, port, dry_run, verbose)
            ...
            success = self._sync_addons_to_server(host, version, port, dry_run, verbose, exclude_cache)
    ...
    def _ensure_remote_dir(self, remote_host: str, remote_dir: str, port: int, dry_run: bool, verbose: bool) -> bool:
        ...
        """
        mkdir_cmd = ["ssh", f"-p{port}", f"root@{remote_host}", f"mkdir -p '{remote_dir}'"]
