...
def setup_github_ssh(
    ...
    key_path: Optional[str] = typer.Option(
        None, "-k", "--key-path", help="Path to local SSH private key for GitHub (default: ~/.ssh/id_rsa)"
    ),
    github_user: Optional[str] = typer.Option(
        None, "-u", "--github-user", help="GitHub username (will prompt if not provided)"
    ),
    generate_key: bool = typer.Option(False, "--generate", help="Generate new SSH key pair for GitHub if not exists"),
    add_to_github: bool = typer.Option(
        False, "--add-to-github", help="Display instructions to add public key to GitHub"
    ),
):
    """
    ...
    if not os.path.exists(key_path) or not os.path.exists(public_key_path):
        if generate_key:
            console.print("[yellow]Generating new SSH key pair for GitHub...[/yellow]")
    ...
    ssh.run(
        f"grep -q 'github.com' {remote_ssh_dir}/known_hosts 2>/dev/null || echo '{github_key}' >> {remote_ssh_dir}/known_hosts",
        sudo=False,
    )
...
"""
    ...
    ssh.run(
        f"echo '{encoded_config}' | base64 -d >> {remote_ssh_dir}/config && chmod 600 {remote_ssh_dir}/config",
        sudo=False,
    )
