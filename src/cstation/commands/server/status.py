...
def server_status(
    hostname: Optional[str] = typer.Argument(
        None, help="Target hostname from inventory (optional - shows all if not specified)"
    ),
    inventory_path: Optional[str] = typer.Option(
        "/etc/cstation/ansible/inventory", "-i", "--inventory", help="Inventory directory path"
    ),
    check_services: bool = typer.Option(
        False, "--services", help="Check common services status (nginx, postgresql, etc.)"
    ),
    show_uptime: bool = typer.Option(True, "--uptime/--no-uptime", help="Include uptime information in status check"),
):
    """
    ...
    if hostname:
        ...
        if not hosts_list:
            console.print("[red]No hosts found in inventory[/red]")
            raise typer.Exit(1)
    ...
    try:
        ...
        cmd = ["ansible", target, "-i", inventory_path, "-m", "setup"]
        ...
        if result.stdout:
            ...
            console.print("[red]Failed to gather system information:[/red]")
            if result.stderr:
        ...
        console.print("[red]ansible command not found. Please install Ansible.[/red]")
        raise typer.Exit(1)
    ...
    if check_services:
        ...
        for service in services:
            try:
                cmd = ["ansible", target, "-i", inventory_path, "-m", "service_facts"]

                result = subprocess.run(cmd, capture_output=True, text=True)

                if result.returncode == 0:
                    console.print("[green]✓ Service check completed[/green]")
                else:
                    console.print("[yellow]⚠ Some services may not be available[/yellow]")
    ...
    if show_uptime:
        ...
        try:
            ...
            cmd = ["ansible", target, "-i", inventory_path, "-m", "command", "-a", "uptime"]
            ...
            if result.returncode == 0 or result.stdout:
                ...
                console.print("[yellow]⚠ Could not retrieve uptime for some hosts[/yellow]")
                if result.stderr:
