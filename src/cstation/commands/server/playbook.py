...
# Create playbook app
playbook_app = typer.Typer(name="playbook", help="Ansible playbook management", invoke_without_command=True)
...
def push_playbook(
    ...
    extra_vars: str = typer.Option(
        "", "--extra-vars", "-e", help="Extra variables to pass to ansible-playbook (e.g., 'pw_version=3.0 env=prod')"
    ),
):
    """
    ...
    try:
        ...
        env = os.environ.copy()
        ansible_cfg_paths = [Path("/etc/cstation/ansible/ansible.cfg"), playbooks_dir / "ansible.cfg"]
