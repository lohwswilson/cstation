...
#### List Available Docker Profiles
...
Deploy a Docker service profile to a specific target host using Ansible.

**Arguments:**
- `profile` - Docker profile name (without .yml extension)
- `target_host` - Target hostname from the Ansible inventory
...
- Service profiles must exist in `/etc/cstation/service/server/` or `/etc/cstation/service/docker/`
...
2. **"Ansible playbook not found"**
   - Verify profile exists: `cstation service server ls` or `cstation service docker ls`
